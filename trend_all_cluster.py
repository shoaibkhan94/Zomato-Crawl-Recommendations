...
while z<10:
    ...
    file_name1='Cui_'+cui+'_'+str(y)+'1.csv'
    with open(file_name,'ab') as f1:
        writeitt = csv.writer(f1)
        writeitt.writerows([month_avg])
    with open(file_name1,'ab') as f2:
        writeittt=csv.writer(f2)
        writeittt.writerows([month_freq])
    os.remove('temp.csv')
    z+=1
...
plt.title('Cluster'+str(z)+' '+cui+' '+str(y))
plt.axis([1,12,0,5])
plt.plot(months,month_avg)
plt.savefig('Cluster_'+str(z)+'_'+cui+'_'+str(y)+'.png')
